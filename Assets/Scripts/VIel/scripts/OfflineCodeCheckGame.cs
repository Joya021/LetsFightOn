...
public class OfflineCodeCheckGame : MonoBehaviour
{
    [Header("References")]
    public OfflineGameManager gameManager;
    public HunterChaseAndHack hunter;
    private PlayerMovement playerMovement;
    private GameObject playerObject;
    public OfflineIntercom linkedIntercom;
    ...
    void Start()
    {
        ...
        {
            linkedIntercom = GetComponent<OfflineIntercom>();
            if (linkedIntercom == null)
            {
    ...
    {
        isOnCooldown = true;
        cooldownTimer = interactCooldown; // Set initial value
        ...
        while (cooldownTimer > 0)
        {
            if (interactCooldownText != null)
                interactCooldownText.text = $"{cooldownTimer:F0}";
            cooldownTimer -= Time.deltaTime; // Update the public variable
            yield return null;
        }

        cooldownTimer = 0f; // Reset to 0
